...
const RING_RADIUS_BASE: float = RADIUS + 10
const RING_RADIUS_STEP: float = 10
const RING_SPEED_BASE: float = 0.8
...
func _draw() -> void:
	...
	for i: int in RING_COUNT:
		...
		var start_angle: float = angle_offset + (passed_time * (RING_SPEED_BASE - (i * RING_SPEED_STEP)))
		var end_angle: float = start_angle + (TAU * RING_ARC_RATIO)
		draw_arc(Vector2.ZERO, RING_RADIUS_BASE + (i * RING_RADIUS_STEP), start_angle, end_angle, 20, color, 3)
